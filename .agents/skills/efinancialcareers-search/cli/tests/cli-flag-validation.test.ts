...
describe("argument validation", () => {
  ...
  test("detail rejects an id it cannot resolve", async () => {
    ...
    expect(r.exitCode).toBe(1)
    expect(r.stderr).toContain("not an eFinancialCareers job id")
  })

  test("--jobage is rejected, not silently ignored", async () => {
    // The site ignores every date filter. Accepting the flag and returning
    // unfiltered results would let a caller believe it had filtered by age.
    const r = await runCLI(["search", "-q", "risk", "--jobage", "7"])
    expect(r.exitCode).toBe(1)
    expect(JSON.parse(r.stderr).code).toBe("UNSUPPORTED_FLAG")
  })

  test("no arguments prints usage and exits 1", async () => {
    const r = await runCLI([])
    expect(r.exitCode).toBe(1)
    expect(r.stdout).toContain("efc-search")
  })
